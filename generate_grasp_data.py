...
from ndf_robot.utils.sim_utils import get_object_clouds, get_clouds, get_object_clouds_from_demo_npz
...
def main(args, global_dict):
    ...
    for i, fname in enumerate(grasp_demo_filenames):
        ...
        print("grasp_obj_pose_world:{}".format(grasp_data['obj_pose_world']))
        print("place_obj_pose_world:{}".format(place_data['obj_pose_world']))
        print("grasp_shapenet_id:{}".format(grasp_data['shapenet_id']))
        print("place_shapenet_id:{}".format(place_data['shapenet_id']))
        log_info("dict(grasp_data).keys():{}".format(dict(grasp_data).keys()))
        log_info("dict(grasp_data)['object_pointcloud'].shape:{}".format(dict(grasp_data)['object_pointcloud'].shape))
        log_info("dict(grasp_data)['object_pointcloud'].shape:{}".format(dict(grasp_data)['object_pointcloud'].shape))
    ...
    grasp_data_0 = np.load(grasp_demo_filenames[0], allow_pickle=True)
    ...
    train_object_ids = []
    for i in range(3):
        log_info("shapenet id of non demo")
        log_info(os.listdir(shapenet_obj_dir)[i])
        log_info(os.listdir(shapenet_obj_dir)[i].split('_')[0])
    ...
    data_dir = '{}_data_{}'.format(test_or_train, args.seed)

    save_dir = os.path.join(os.getcwd(),data_dir,'renders')
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)

    log_info(f'CREATING {test_or_train} DATA WITH SEED {args.seed}..........................')
    viz_data_list_test = []
    ...
    for iteration in range(args.start_iteration, args.num_iterations):
        ...
        # _,_,_ = get_object_clouds_from_demo_npz(grasp_data_0)
        ...
        viz_data_list_test.append(viz_dict)
        viz_sample_fname = osp.join(eval_iter_dir, 'overlay_visualization_data.npz')
        np.savez(viz_sample_fname, viz_dict=viz_dict, viz_data_list_test=viz_data_list_test)
        ...
        if grasp_success:
            ...
            np.savez(f'{data_dir}/renders/{iteration}_post_grasp_all_points.npz', 
                    clouds = cloud_points, colors = cloud_colors, classes = cloud_classes, shapenet_id = obj_shapenet_id)

            np.savez(f'{data_dir}/renders/{iteration}_post_grasp_obj_points.npz', 
                    clouds = obj_points, colors = obj_colors, classes = obj_classes, shapenet_id = obj_shapenet_id)
        ...
        if grasp_success:
            ...
            if place_success:
                cloud_points, cloud_colors, cloud_classes = get_clouds(cams)
                obj_points, obj_colors, obj_classes = get_object_clouds(cams)   
                np.savez(f'{data_dir}/renders/{iteration}_post_place_all_points.npz', 
                        clouds = cloud_points, colors = cloud_colors, classes = cloud_classes, shapenet_id = obj_shapenet_id)
                np.savez(f'{data_dir}/renders/{iteration}_post_place_obj_points.npz', 
                        clouds = obj_points, colors = obj_colors, classes = obj_classes, shapenet_id = obj_shapenet_id)
        ...
        cloud_points, cloud_colors, cloud_classes = get_clouds(cams)
        obj_points, obj_colors, obj_classes = get_object_clouds(cams)
        np.savez(f'{data_dir}/renders/{iteration}_final_all_points.npz', 
                clouds = cloud_points, colors = cloud_colors, classes = cloud_classes, shapenet_id = obj_shapenet_id)
        np.savez(f'{data_dir}/renders/{iteration}_final_obj_points.npz', 
                clouds = obj_points, colors = obj_colors, classes = obj_classes, shapenet_id = obj_shapenet_id)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='multi_category_weights')
