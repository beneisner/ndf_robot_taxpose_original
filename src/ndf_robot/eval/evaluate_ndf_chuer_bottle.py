...
from equivariant_pose_graph.models.transformer_flow import ResidualFlow, ResidualFlow_V1, \
    ...
    ResidualFlow_DiffEmbTransformerSymmetryBreaking, CorrespondenceFlow_DiffEmbMLP
from equivariant_pose_graph.utils.se3 import pure_translation_se3
import os
...
GRASP_Z_ADJUSTMENT = 0  # -0.05  # -0.01  # -0.01
PLACE_ADJUSTMENT = 0.02
RADIAL_SHIFT = 0  # 0.3
...
def get_world_transform(pred_T_action_mat, obj_start_pose, point_cloud, invert=False):
    ...
    centered_pose = util.transform_pose(
        pose_source=obj_start_pose, pose_transform=centering_pose)  # obj_start_pose: stamped_pose
...
@hydra.main(config_path="/home/exx/Documents/equivariant_pose_graph/configs/exx/", config_name="exx_dcpflow_residual0_test_ndf_my_model_bottle")
def main(hydra_cfg):
    cur_time = datetime.datetime.today().strftime('%Y-%m-%d-%H-%M-%S')
    txt_file_name = 'bottle_anypose_{}_{}_.txt'.format(
        hydra_cfg.any_pose, cur_time)
    log_dir = os.path.join(os.getcwd(), cur_time, 'txt_logs')
    if not os.path.isdir(log_dir):
        os.makedirs(log_dir)

    txt_file_name = os.path.join(log_dir, txt_file_name)
    log_info("txt_file_name")
    log_info(txt_file_name)
    log_info("GRASP_Z_ADJUSTMENT")
    log_info(GRASP_Z_ADJUSTMENT)
    log_info("PLACE_ADJUSTMENT")
    log_info(PLACE_ADJUSTMENT)
    log_info("RADIAL_SHIFT")
    log_info(RADIAL_SHIFT)
    log_info("log_dir")
    log_info(log_dir)
    f = open(txt_file_name, 'a')
    f.write("seed:{}\n".format(hydra_cfg.seed))
    f.write("GRASP_Z_ADJUSTMENT:{}\n".format(GRASP_Z_ADJUSTMENT))
    f.write("PLACE_ADJUSTMENT:{}\n".format(PLACE_ADJUSTMENT))
    f.write("RADIAL_SHIFT:{}\n".format(RADIAL_SHIFT))

    data_dir = hydra_cfg.data_dir
    save_dir = os.path.join(os.getcwd(), cur_time, 'renders')
    ...
    if obj_class == 'mug':
        ...
        avoid_shapenet_ids.append('56c23ba1699f6294435b5a0263ddd2e2')
    else:
        test_shapenet_ids = []
        ...
        if hydra_cfg.diff_emb:
            if hydra_cfg.diff_transformer:
                if hydra_cfg.mlp:
                    ...
                    network = ResidualFlow_DiffEmbTransformerSymmetryBreaking(
    ...
    if(hydra_cfg.checkpoint_file_place is not None):
        place_model.load_state_dict(torch.load(
            hydra_cfg.checkpoint_file_place)['state_dict'])
    ...
    if hydra_cfg.checkpoint_file_place_refinement is not None:
        ...
        place_model_refinement.load_state_dict(torch.load(
            hydra_cfg.checkpoint_file_place_refinement)['state_dict'])
        log_info("Place Refinement Model Loaded from " +
    ...
    if(hydra_cfg.checkpoint_file_grasp is not None):
        grasp_model.load_state_dict(torch.load(
            hydra_cfg.checkpoint_file_grasp)['state_dict'])
    ...
    if hydra_cfg.checkpoint_file_grasp_refinement is not None:
        ...
        if(hydra_cfg.checkpoint_file_grasp_refinement is not None):
            grasp_model_refinement.load_state_dict(torch.load(
                hydra_cfg.checkpoint_file_grasp_refinement)['state_dict'])
            log_info("Model Grasp Refinement Model Loaded from " +
    ...
    for iteration in range(hydra_cfg.start_iteration, hydra_cfg.num_iterations):
        ...
        points_gripper_raw, points_mug_raw = load_data_raw(
            num_points=1024, clouds=obj_points, classes=obj_classes, action_class=2, anchor_class=0)
        points_mug, points_rack = load_data(
            num_points=1024, clouds=obj_points, classes=obj_classes, action_class=0, anchor_class=1)
        # if hydra_cfg.sym_cls:
        #     symmetric_cls = get_sym_label(
        #         action_cloud=points_mug, anchor_cloud=points_rack, action_class=0, anchor_class=1)
        #     symmetric_cls = symmetric_cls.unsqueeze(-1)
        #     points_mug = torch.cat(
        #         [points_mug, symmetric_cls], axis=-1)
        #     points_rack = torch.cat(
        #         [points_rack, torch.ones(symmetric_cls.shape).to(points_rack.device)], axis=-1)

        ans = place_model.get_transform(
            points_rack, points_mug, points_rack, action_class=0, anchor_class=1)  # 1, 4, 4

        if hydra_cfg.checkpoint_file_place_refinement is not None:
            pred_points_action = ans["pred_points_action"]
            pred_T_action = ans["pred_T_action"]
            points_trans_action, points_trans_anchor, points_action_mean = place_model.action_centered(
                pred_points_action, points_rack)
            ...
            ans_refinement = place_model_refinement.get_transform(
                points_rack, points_trans_action, points_trans_anchor, action_class=0, anchor_class=1)
            pred_T_action = pred_T_action.compose(T_trans.inverse().compose(
                ans_refinement["pred_T_action"]).compose(T_trans))
        ...
        obj_pose_world = util.list2pose_stamped(
            list(obj_pose_world[0]) + list(obj_pose_world[1]))  # stamped_pose
        obj_start_pose = obj_pose_world
        rack_relative_pose = get_world_transform(
            pred_T_action_mat, obj_start_pose, points_mug_raw)  # pose_stamped
        obj_end_pose_list = util.pose_stamped2list(rack_relative_pose)
        obj_end_pose_list[1] += PLACE_ADJUSTMENT
        # obj_end_pose_list[2] += 0.01
        ...
        # if hydra_cfg.sym_cls:
        #     symmetric_cls = get_sym_label(
        #         action_cloud=points_gripper, anchor_cloud=points_mug, action_class=2, anchor_class=0)

        #     symmetric_cls = symmetric_cls.unsqueeze(-1)
        #     points_mug = torch.cat(
        #         [points_mug, symmetric_cls], axis=-1)
        #     points_gripper = torch.cat(
        #         [points_gripper, torch.ones(symmetric_cls.shape).to(points_gripper.device)], axis=-1)

        points_mug = grasp_model.adjust_points_along_z(
            points_mug, z_shift=GRASP_Z_ADJUSTMENT, object_type='bottle')
        ans_grasp = grasp_model.get_transform(
            points_rack, points_gripper, points_mug, action_class=2, anchor_class=0)
        points_mug = grasp_model.adjust_points_along_radial(points_action=points_gripper,
                                                            points_anchor=points_mug,
                                                            ans_dict=ans_grasp, radial_shift=RADIAL_SHIFT)

        ans_grasp = grasp_model.get_transform(
            points_rack, points_gripper, points_mug, action_class=2, anchor_class=0)  # 1, 4, 4
        pred_T_action_init_gripper2mug = ans_grasp["pred_T_action"]
        pred_T_action_mat_gripper2mug = pred_T_action_init_gripper2mug.get_matrix()[
            0].T.detach().cpu().numpy()

        # pred_T_action_mat_gripper2mug[2, -1] -= 0.02

        gripper_relative_pose = get_world_transform(
            pred_T_action_mat_gripper2mug, ee_pose_world, points_gripper_raw)  # transform from gripper to mug in world frame
        pre_grasp_ee_pose = util.pose_stamped2list(gripper_relative_pose)
        # pre_grasp_ee_pose[2] += 0.02
        ...
        pregrasp_offset_tf = get_ee_offset(
            ee_pose=pre_grasp_ee_pose)
        # pre_pre_grasp_ee_pose = pre_grasp_ee_pose
        pre_pre_grasp_ee_pose = util.pose_stamped2list(util.transform_pose(pose_source=util.list2pose_stamped(
            pre_grasp_ee_pose), pose_transform=util.list2pose_stamped(pregrasp_offset_tf)))
        ...
        np.savez(f'{save_dir}/{iteration}_post_teleport_all_points.npz',
                 clouds=cloud_points, colors=cloud_colors, classes=cloud_classes, shapenet_id=obj_shapenet_id)

        np.savez(f'{save_dir}/{iteration}_post_teleport_obj_points.npz',
                 clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id)
        ...
        for g_idx in range(2):
            ...
            if jnt_pos is None or grasp_jnt_pos is None:
                log_info("jnt_pos:{}, grasp_jnt_pos:{}".format(
                    jnt_pos == None, grasp_jnt_pos == None))
            ...
            if grasp_jnt_pos is not None and jnt_pos is not None:
                if g_idx == 0:
                    ...
                    np.savez(f'{save_dir}/{iteration}_pre_grasp_all_points.npz',
                             clouds=cloud_points, colors=cloud_colors, classes=cloud_classes, shapenet_id=obj_shapenet_id)

                    np.savez(f'{save_dir}/{iteration}_pre_grasp_obj_points.npz',
                             clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id)
                ...
                if grasp_plan is None:
                    # robot.arm.set_jpos(jnt_pos, ignore_physics=True)
                    ...
                    # obj_points, obj_colors, obj_classes = get_object_clouds(
                    #     cams)
                    # np.savez(f'{save_dir}/{iteration}_plan1_obj_points.npz',
                    #          clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id)
                    ...
                    # obj_points, obj_colors, obj_classes = get_object_clouds(
                    #     cams)
                    # np.savez(f'{save_dir}/{iteration}_plan2_obj_points.npz',
                    #          clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id)

                    log_info("plan1:{}, plan2:{}".format(
                        plan1 != None, plan2 != None))
                    # import pdb
                    # pdb.set_trace()
                    if plan1 is not None and plan2 is not None:
                        ...
                        np.savez(f'{save_dir}/{iteration}_post_grasp_all_points.npz',
                                 clouds=cloud_points, colors=cloud_colors, classes=cloud_classes, shapenet_id=obj_shapenet_id)

                        np.savez(f'{save_dir}/{iteration}_post_grasp_obj_points.npz',
                                 clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id)
        ...
        if grasp_success:
            ...
            ee_end_pose = util.transform_pose(pose_source=util.list2pose_stamped(
                pre_grasp_ee_pose), pose_transform=transform_rack_relative_pose)
            ...
            place_jnt_pos = ik_helper.get_feasible_ik(
                ee_end_pose_list)
        ...
        kvs['overall success Rate'] = overall_success_num / \
            float(len(grasp_success_list))
    ...
    f = open(txt_file_name, "a")
    f.write("Place Success Rate:{}".format(sum(
            place_success_list) / float(len(place_success_list))))
    f.write("Place [teleport] Success Rate: {}".format(sum(
            place_success_teleport_list) / float(len(place_success_teleport_list))))
    f.write("Grasp Success Rate:{}".format(sum(
            grasp_success_list) / float(len(grasp_success_list))))
    f.close()
