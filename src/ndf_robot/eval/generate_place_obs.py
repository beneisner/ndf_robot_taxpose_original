...
@hydra.main(config_path="../config/eval_cfgs", config_name="exx_dcpflow_residual0") 
def get_model(cfg):
...
def main(args, global_dict):
    ...
    # for iteration in[3,4,8,10,14]:
        ...
        # TODO: relace rack_relative_pose with model predicted
        ...
        data_dir = 'debug_place_shapeid_34ae0b61b0d8aaf2d7b20fded0142d7a'
        ...
        # pred_T_action, pred_T_anchor, pred_T  = place_model(points_action, points_anchor) # 1, 4, 4
        # # M = [
        # #         [Rxx, Ryx, Rzx, 0],
        # #         [Rxy, Ryy, Rzy, 0],
        # #         [Rxz, Ryz, Rzz, 0],
        # #         [Tx,  Ty,  Tz,  1],
        # #     ]
        # pred_T_action = torch.transpose(pred_T_action,-1,-2).detach().cpu().numpy()
  
        # log_info("pred_T_action.shape")
        # log_info(pred_T_action.shape)
        # rack_relative_pose = util.pose_stamped2list(util.pose_from_matrix(pred_T_action[0]))
        ...
        np.savez(f'{save_dir}/{iteration}_teleport_obj_points.npz', 
                 clouds = obj_points, colors = obj_colors, classes = obj_classes, shapenet_id = obj_shapenet_id)# , pred_T_action = pred_T_action)
        log_info("Saved point cloud data to:")
        log_info(f'{save_dir}/{iteration}_teleport_obj_points.npz')
        ...
        for g_idx in range(2):
            ...
            if grasp_jnt_pos is not None and jnt_pos is not None: # both ik returns a grapable joint plan for pre_pre_grasp and pre_grasp
                if g_idx == 0:
                    robot.pb_client.set_step_sim(True)
                ...
                if grasp_plan is None:
                    ...
                    if plan1 is not None and plan2 is not None:
                        ...
                        grasped_img_fname = osp.join(eval_grasp_imgs_dir, '{}_pre_grasped_no_collision.png'.format(iteration))
                        ...
                        grasped_rgb = robot.cam.get_images(get_rgb=True)[0]
                        grasped_img_fname = osp.join(eval_grasp_imgs_dir, '{}_pre_grasped.png'.format(iteration))
                        np2img(grasped_rgb.astype(np.uint8), grasped_img_fname)
                        ...
                        time.sleep(1.0)
        grasped_rgb = robot.cam.get_images(get_rgb=True)[0]
        grasped_img_fname = osp.join(eval_grasp_imgs_dir, '{}_grasped.png'.format(iteration))
        np2img(grasped_rgb.astype(np.uint8), grasped_img_fname)
        if grasp_success:
            ####################################### get place pose ###########################################
            ...
            placed_img_fname = osp.join(eval_place_imgs_dir, '{}_placed_cam_{}.png'.format(iteration, i))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_data_dir', type=str, default='test_place_model')
