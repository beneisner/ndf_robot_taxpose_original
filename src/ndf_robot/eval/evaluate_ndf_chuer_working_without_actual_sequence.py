...
def main(hydra_cfg):
    ...
    for iteration in range(hydra_cfg.start_iteration, hydra_cfg.num_iterations):
        ...
        pred_T_action_mat_gripper2mug = pred_T_action_init_gripper2mug.get_matrix()[
            0].T.detach().cpu().numpy()
        pred_T_action_mat_gripper2mug[2, -1] -= 0.005
        ...
        np.savez(f'{save_dir}/{iteration}_init_obj_points.npz',
                 ...
                 points_mug=points_mug.detach().cpu(),
                 points_gripper=points_gripper.detach().cpu(),
                 points_rack=points_rack.detach().cpu(),
        ...
        ee_end_pose = util.transform_pose(pose_source=util.list2pose_stamped(
            pre_grasp_ee_pose), pose_transform=rack_relative_pose)
        ...
        if grasp_success:
            ...
            grasp_rgb = robot.cam.get_images(get_rgb=True)[
                0]
            grasp_img_fname = osp.join(
                eval_grasp_imgs_dir, 'get_place_pose_%d.png' % iteration)
            np2img(grasp_rgb.astype(
                np.uint8), grasp_img_fname)
