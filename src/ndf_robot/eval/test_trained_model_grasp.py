...
def get_world_transform(pred_T_action_mat, obj_start_pose):
    """
...
def main(hydra_cfg):
    ...
    for iteration in range(hydra_cfg.start_iteration, hydra_cfg.num_iterations):
        ...
        np.savez(f'{save_dir}/{iteration}_init_obj_points.npz',
                 clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id,  pred_T_action_transformed=pred_T_action_transformed.detach().cpu(), pred_T_action_mat=pred_T_action_mat, points_action=points_action.detach().cpu(), points_anchor=points_anchor.detach().cpu())
        ...
        rack_relative_pose_world = get_world_transform(
            pred_T_action_mat, obj_start_pose)  # pose_stamped
        obj_end_pose_list = util.pose_stamped2list(rack_relative_pose_world)

        ee_end_pose = util.transform_pose(pose_source=util.list2pose_stamped(
            pre_grasp_ee_pose), pose_transform=rack_relative_pose_world)
