...
from equivariant_pose_graph.models.transformer_flow import ResidualFlow, ResidualFlow_DiffEmbTransformer, ResidualFlow_V1, \
...
def main(args, global_dict):
    ...
    for iteration in range(args.start_iteration, args.num_iterations):
        ...
        obj_pose_world = robot.pb_client.get_body_state(obj_id)[:2]
        ...
        pred_T_action_transformed = pred_T_action_init.transform_points(points_action)
        ...
        np.savez(f'{save_dir}/{iteration}_init_obj_points.npz', 
                 clouds = obj_points, colors = obj_colors, classes = obj_classes, shapenet_id = obj_shapenet_id, pred_T_action_init = pred_T_action_init.get_matrix().detach().cpu())
        log_info("Saved point cloud data to:")
        log_info(f'{save_dir}/{iteration}_init_obj_points.npz')
 

        obj_end_pose = util.transform_pose(pose_source=obj_start_pose, pose_transform=util.pose_from_matrix(T_pred))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='place_test_0_my_model_may11_overfit_closetoaccpred')
    parser.add_argument('--eval_data_dir', type=str, default='test_place_model_overfit_my_model_may10')
    ...
    parser.add_argument('--any_pose', action='store_true', default=False)
    ...
    parser.add_argument('--just_teleport_place', default = True)
