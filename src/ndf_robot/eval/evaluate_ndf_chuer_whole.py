...
@hydra.main(config_path="/home/exx/Documents/equivariant_pose_graph/configs/exx/", config_name="exx_dcpflow_residual0_test_ndf_my_model")
def main(hydra_cfg):
    ...
    for iteration in range(hydra_cfg.start_iteration, hydra_cfg.num_iterations):
        ...
        place_success_teleport_list.append(1)
        ...
        if grasp_success:
            ...
            if place_jnt_pos is not None and pre_place_jnt_pos2 is not None and pre_place_jnt_pos1 is not None:
                ...
                ################################################################################################################
                    ...
                    grasp_rgb = robot.cam.get_images(get_rgb=True)[0]
                    grasp_img_fname = osp.join(
                        eval_grasp_imgs_dir, 'post_place_%d.png' % iteration)
                    np2img(grasp_rgb.astype(np.uint8), grasp_img_fname)
                    cloud_points, cloud_colors, cloud_classes = get_clouds(
                        cams)
                    obj_points, obj_colors, obj_classes = get_object_clouds(
                    ...
                    np.savez(f'{save_dir}/{iteration}_post_place_all_points.npz',
                             clouds=cloud_points, colors=cloud_colors, classes=cloud_classes, shapenet_id=obj_shapenet_id)

                    np.savez(f'{save_dir}/{iteration}_post_place_obj_points.npz',
                             clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id)
