...
from equivariant_pose_graph.models.transformer_flow import ResidualFlow, ResidualFlow_V1, \
    ...
    ResidualFlow_Identity, ResidualFlow_PE, ResidualFlow_DiffEmb, CorrespondenceFlow_DiffEmbMLP
...
GRASP_Z_ADJUSTMENT = -0.03
PLACE_ADJUSTMENT = -0.002
...
@hydra.main(config_path="/home/exx/Documents/equivariant_pose_graph/configs/exx/", config_name="exx_dcpflow_residual0_test_ndf_my_model_bowl")
def main(hydra_cfg):

    cur_time = datetime.datetime.today().strftime('%Y-%m-%d-%H-%M-%S')
    txt_file_name = 'bowl_anypose_{}_{}_.txt'.format(
        hydra_cfg.any_pose, cur_time)
    log_dir = os.path.join(os.getcwd(), 'txt_logs')
    ...
    for iteration in range(hydra_cfg.start_iteration, hydra_cfg.num_iterations):
        ...
        ans = place_model.get_transform(
            points_mug, points_rack, action_class=0, anchor_class=1)   # 1, 4, 4
        ...
        if hydra_cfg.checkpoint_file_place_refinement is not None:
            ...
            ans_refinement = place_model_refinement.get_transform(
                points_trans_action, points_trans_anchor, action_class=0, anchor_class=1)
        ...
        obj_end_pose_list[1] += PLACE_ADJUSTMENT  # 0.005
        ...
        points_mug = grasp_model.adjust_points_along_z(
            points_mug, z_shift=GRASP_Z_ADJUSTMENT)

        ans_grasp = grasp_model.get_transform(
            points_gripper, points_mug, action_class=2, anchor_class=0)  # 1, 4, 4
        ...
        # pred_T_action_mat_gripper2mug[2, -1] += GRASP_Z_ADJUSTMENT
