...
def main(args, global_dict):
    ...
    # test_object_ids = train_ids
    test_object_ids = ['649a51c711dc7f3b32e150233fdd42e9']
    ...
    for iteration in range(args.start_iteration, args.num_iterations):
        ...
        # load a test object
        obj_shapenet_id = '649a51c711dc7f3b32e150233fdd42e9'
        ...
        obj_points, obj_colors, obj_classes = get_object_clouds(
            cams, occlusion=args.occlusion)
        if place_success_teleport:
            np.savez(f'{data_dir}/renders/{iteration}_afterteleport_all_points.npz',
                     clouds=cloud_points, colors=cloud_colors, classes=cloud_classes, shapenet_id=obj_shapenet_id)

            np.savez(f'{data_dir}/renders/{iteration}_afterteleport_obj_points.npz',
                     clouds=obj_points, colors=obj_colors, classes=obj_classes, shapenet_id=obj_shapenet_id)

            delta_T = util.get_se3(util.list2pose_stamped(rack_relative_pose))
            rack_relative_pose_list.append(
                delta_T)
        else:
            failed_teleport_shapeids.append(obj_shapenet_id)
